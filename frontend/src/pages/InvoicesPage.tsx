...
export default function InvoicesPage({
  ...
  const invoiceSidebarSections = useMemo<SidebarSection[]>(() => {
    ...
    const typeItems = TYPE_FILTERS.map((filter) => {
      const IconComponent = filter.icon;
      const count =
        filter.key === ""
    ...
    return [
      { id: "invoice-status", title: "STATUS", items: statusItems },
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            .meta .row { display: grid; grid-template-columns: 120px 1fr; gap: 2px; }
            ...
            <div className="invoice-detail-toolbar mb-3">
              ...
              <div className="invoice-toolbar-actions">
                ...
                {!newMode && selectedInvoice && (() => {
                  const allKeys   = ["draft", "posted", "fiscalized", "paid"];
                  const stageKeys   = ["draft", "posted", "paid"];
                  const stageLabels = ["Draft", "Posted", "Paid"];
                  const rawIdx    = allKeys.indexOf(statusLabel ?? "");
                  // fiscalized (idx 2) maps to posted bucket (idx 1)
                  const currentIdx = rawIdx <= 1 ? rawIdx : rawIdx - 1;
                  return (
                    <div className="invoice-statusbar">
                      {stageKeys.map((key, i) => {
                        ...
                        return (
                          <div
                            key={key}
                            className={`invoice-stage${
                              isActive ? " invoice-stage--active" :
                              isDone   ? " invoice-stage--done"   : ""
                            }`}
                          >
                            {stageLabels[i]}
                          </div>
